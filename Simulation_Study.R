  ...
  for(i in 1:100){
    ...
    if(copula == "clayton"){
      repeat{
        ...
        copEST = try(fitCopula(copula = claytonCopula(), data = sample), silent = T)
    ...
    if(copula == "gumbel"){
      repeat{
        ...
        #Step 2: Estimate copula
        copEST = try(fitCopula(copula = gumbelCopula(), data = sample), silent = T)
        if(class(copEST) != "try-error"){break}
      }
  ...
  for(i in 1:100){
    if(copula == "clayton"){
      repeat{
        ...
        #Step 2: Estimate copula
        copEST = try(fitCopula(copula = claytonCopula(dim = 3), data = sample), silent = T)
        if(class(copEST) != "try-error"){break}
      }
    ...
    if(copula == "gumbel"){
      repeat{
        ...
        #Step 2: Estimate copula
        copEST = try(fitCopula(copula = gumbelCopula(dim = 3), data = sample), silent = T)
        if(class(copEST) != "try-error"){break}
      }
...
rownames(fin05) = rownames(fin01) = c("Bivariate", "Multi", "Vulnerability")
colnames(fin05) = colnames(fin01) = rep(c("Clayton", "Gumbel"),3)
xtable(fin05, digits = 4)
xtable(fin01, digits = 4)
