...
getAllARMA = function(ret, ar.max = 6, ma.max = 6){
  ...
  for(i in seq_len(nrow(table))){
    ...
    fit = try(arfimafit(spec, data = as.vector(ret), solver = "hybrid"))
    ...
    tmp = try(Box.test(res, lag = max(df+1, 8), fitdf = df, type = c("Ljung-Box"))$p.value)
    if(class(tmp)!= "try-error"){
      table$LB[i] = tmp  
    }
    
    tmp = try(Box.test(res^2, lag = max(df+1, 8), fitdf = df, type = c("Ljung-Box"))$p.value)
    if(class(tmp)!= "try-error"){
      table$LBsq[i] = tmp  
    }
...
getAllARMAGARCH = function(ret, ar.max = 6, ma.max = 6, arch.max = 6, garch.max = 1, model = "sGARCH"){
  ...
  for(i in seq_len(nrow(table))){
    ...
    fit = try(ugarchfit(spec, data = as.vector(ret), solver = "hybrid"))
    if(class(fit) == "try-error"){next}
    ...
    tmp = try(Box.test(stdres, lag = max(df+1, 8), fitdf = df, type = c("Ljung-Box"))$p.value)
    if(class(tmp)!= "try-error"){
      table$LB[i] = tmp  
    }
    
    tmp = try(Weighted.LM.test(res, h.t = h.t, weighted = T, lag = max(gdf+1, 8), type = "correlation", fitdf = gdf)$p.value)
    if(class(tmp)!= "try-error"){
      table$WLM[i] = tmp  
    }
...
## Step 5: Calculate all possible ARMA(p,q)-GJR-GARCH(P,Q) models and consider those fulfilling LB, WLM, SB ##
