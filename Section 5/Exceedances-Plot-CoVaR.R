...
evalBivariate = function(retX, retY, VaRX, CoVaR){
  ...
  CoVaR_diff = rep(0, length(CoVaR))
  CoVaR_diff[CoVaR_exc] = abs(retY[CoVaR_exc] - CoVaR[CoVaR_exc])
...
evalMulti3X = function(retX1, retX2, retX3, retY, VaRX1, VaRX2, VaRX3, MCoVaR){
  ...
  MCoVaR_diff = rep(0, length(MCoVaR))
  MCoVaR_diff[MCoVaR_exc] = abs(retY[MCoVaR_exc] - MCoVaR[MCoVaR_exc])
...
evalVulnerability3X = function(retX1, retX2, retX3, retY, VaRX1, VaRX2, VaRX3, VCoVaR){
  ...
  VCoVaR_diff = rep(0, length(VCoVaR))
  VCoVaR_diff[VCoVaR_exc] = abs(retY[VCoVaR_exc] - VCoVaR[VCoVaR_exc])
  ...
  if (nser > 1 && (plot.type == "multiple" || ngraph > 
    ...
    for (j in seq_along(levels(screens))) {
      ...
      if (j%%nr == 0 || j == length(levels(screens))) {
        ...
        args$type <- "n"
        do.call("plot", args)
        mtext(xlab, side = 1, line = 3, cex = 0.75)
      }
      else {
      ...
      corners = par("usr")
      par(xpd = TRUE)
      text(x = corners[1]-300, y = mean(corners[3:4])-0.2, ylab[[j]], cex = 0.9, pos = 4)
...
NAfill = VecToTS(rep(NA, length(rBTC)))
resCOVAR = cbind(VecToTS(norm_CoVaR_LTC_exc), VecToTS(norm_CoVaR_XMR_exc), VecToTS(norm_CoVaR_XRP_exc), VecToTS(norm_CoVaR_Sys_exc), VecToTS(norm_CoVaR_Multi_exc), VecToTS(norm_CoVaR_V_exc), NAfill,
                 VecToTS(t_CoVaR_LTC_exc), VecToTS(t_CoVaR_XMR_exc), VecToTS(t_CoVaR_XRP_exc), VecToTS(t_CoVaR_Sys_exc), VecToTS(t_CoVaR_Multi_exc), VecToTS(t_CoVaR_V_exc), NAfill,
                 VecToTS(Patton_CoVaR_LTC_exc), VecToTS(Patton_CoVaR_XMR_exc), VecToTS(Patton_CoVaR_XRP_exc), VecToTS(Patton_CoVaR_Sys_exc), NAfill,
                 VecToTS(DCC_CoVaR_LTC_exc), VecToTS(DCC_CoVaR_XMR_exc), VecToTS(DCC_CoVaR_XRP_exc), VecToTS(DCC_CoVaR_Sys_exc), VecToTS(DCC_CoVaR_Multi_exc), VecToTS(DCC_CoVaR_V_exc), NAfill,
                 VecToTS(Clayton_CoVaR_LTC_exc), VecToTS(Clayton_CoVaR_XMR_exc), VecToTS(Clayton_CoVaR_XRP_exc), VecToTS(Clayton_CoVaR_Sys_exc), VecToTS(Clayton_CoVaR_Multi_exc), VecToTS(Clayton_CoVaR_V_exc), NAfill,
                 VecToTS(Gumbel_CoVaR_LTC_exc), VecToTS(Gumbel_CoVaR_XMR_exc), VecToTS(Gumbel_CoVaR_XRP_exc), VecToTS(Gumbel_CoVaR_Sys_exc), VecToTS(Gumbel_CoVaR_Multi_exc), VecToTS(Gumbel_CoVaR_V_exc))
colnames(resCOVAR) = rep(c("CoVaR|LTC", "CoVaR|XMR", "CoVaR|XRP", "SCoVaR", "MCoVaR", "VCoVaR", ""), 6)[-c(19,20,42)]

#standardize exceedances separately
resCOVAR = resCOVAR/max(resCOVAR, na.rm = T)

#CoVaR exceedances
col = c(rep("red3", 7), rep("darkgreen", 7), rep(hcl.colors(6, palette = "Berlin")[4],5),
        rep(hcl.colors(6, palette = "Berlin")[2],7), rep("blue3", 7), rep("black", 6))
plot(resCOVAR, nc = 1, yaxt = "n", las = 1, bty = "n", col = as.list(col), lwd = 1.7,
     ylim = c(0,1), yaxs = "i", xlab = "Time", main = "Copulae from top to bottom: Normal | t | Patton-t | DCC-t | Clayton | Gumbel", las = 1)
